...
**목표**: Scheduler, Crawler, Summarizer, Validator, Notifier 5명의 에이전트가 협력해서 AI 뉴스 수집 → 요약 → Slack 발송 자동화

**배울 것**:
- **멀티에이전트 아키텍처**: 5명의 에이전트가 각자 역할 수행
- **역할 분담**: 입력/처리/검증/출력의 파이프라인
- **상태 관리**: `mission.md`로 전체 진행도 추적
- **비용 최적화**: 오픈소스 모델로 저비용 운영
- **안전한 무한 루프**: 5라운드 제한으로 행(hang) 방지

**시간 분배**:
- 설정 확인 및 이해: 1분
- 실행 및 모니터링: 4분

→ [실습 5 상세 가이드](./practice-5-opencode-agent/README.md)
